        ...
        */
        ...
        $subject_data = $CI->db->get(db_prefix().'projects')->result_array();
        $subject = $inbox['subject'];
        
        foreach($subject_data as $sub_data){
        
        $project_key = $sub_data['project_key'];
        $result = str_contains($subject, $project_key);
        if($result){
        
        
        $pro['rel_id'] = $sub_data['id'];
        $pro['mail_id'] = $inbox_id;
        $pro['from_module'] = 'Mailbox';
        $pro['rel_type'] = 'projects';
        $pro['assign_date'] = date('Y-m-d H:i:sa');
        
        
        
        $CI->db->insert(db_prefix().'mail_assign', $pro);
