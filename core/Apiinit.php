...
class Apiinit
{
    ...
    public static function pre_validate($module_name, $code = "")
    {
        ...
        foreach ($all_activated as $active_module => $value) {
            if (option_exists($module_name . '_verification_id') && !empty(get_option($active_module . '_verification_id'))) {
                ...
                $id_data         = explode('|', $verification_id);
                if ($id_data[3] == $code) {
                    return ['status' => false, 'message' => 'Purchase code used in other module'];
                }
            }
        }
